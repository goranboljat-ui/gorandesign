...
const reviews = [
  { name: "Seamus C.", text: "The smallest pub in Cork and somehow the best. Mutton Lane is one of a kind — you're shoulder to shoulder with strangers who become friends in minutes. Magical." },
  { name: "Brigid O'L.", text: "If you visit Cork and don't go to Mutton Lane, did you even visit Cork? The history, the atmosphere, the perfect pint — it's unlike anywhere else." },
  { name: "Tomás F.", text: "Been coming here for 20 years. Nothing has changed and nothing should. Cork's finest pub, full stop." },
];
...
export default function MuttonLaneInnV2() {
  const isMobile = useIsMobile();
  const [rev, setRev] = useState(0);
  const [menuOpen, setMenuOpen] = useState(false);
  useEffect(() => {
    const t = setInterval(() => setRev(i => (i + 1) % reviews.length), 5000);
  ...
  return (
    <div style={{ background: DARK, color: CREAM, fontFamily: "'Georgia', serif", minHeight: "100vh" }}>
      ...
      <section style={{ paddingTop: 62 }}>
        <div style={{ position: "relative", height: isMobile ? "60vh" : "70vh", overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: isMobile ? "2rem 1.5rem" : "3rem 4rem" }}>
            ...
            <h1 style={{ fontSize: isMobile ? "clamp(38px,11vw,68px)" : "clamp(48px,8vw,92px)", fontWeight: 900, lineHeight: 0.9, margin: "0 0 20px", letterSpacing: -2, fontStyle: "italic" }}>
              Mutton Lane<br /><span style={{ color: AMBER2, fontStyle: "normal" }}>Inn.</span>
            </h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 4 }}>
          {gallery.map((img, i) => (
            <div key={i} style={{ position: "relative", height: isMobile ? 180 : 250, overflow: "hidden" }}>
              ...
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(transparent, rgba(10,8,6,0.8))", padding: "12px 14px" }}>
                <span style={{ fontSize: 11, color: CREAM, letterSpacing: "0.15em", textTransform: "uppercase" }}>{img.label}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <section style={{ background: DARK, padding: isMobile ? "60px 20px" : "90px 24px" }}>
        <div style={{ maxWidth: 660, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "center", marginTop: 28 }}>
            {reviews.map((_, i) => (
              <button key={i} onClick={() => setRev(i)} style={{ width: i === rev ? 28 : 8, height: 8, borderRadius: 4, background: i === rev ? AMBER : "rgba(244,234,216,0.2)", border: "none", cursor: "pointer", transition: "all .3s" }} />
            ))}
          </div>
        </div>
      </section>
            ...
            <div style={{ background: DARK, borderRadius: 4, padding: "28px", border: "1px solid rgba(200,120,10,0.15)" }}>
              ...
              <div style={{ marginTop: 20 }}>
                ...
                <div style={{ display: "flex", gap: 20, marginTop: 16, flexWrap: "wrap" }}>
                  <a href="https://maps.google.com/?q=Mutton+Lane+Inn+3+Mutton+Lane+Cork" target="_blank" rel="noopener noreferrer" style={{ color: SMOKE, textDecoration: "none", fontSize: 13 }}>📍 Google Maps</a>
                  <a href="https://www.facebook.com/mutton.lane/" target="_blank" rel="noopener noreferrer" style={{ color: SMOKE, textDecoration: "none", fontSize: 13 }}>📘 Facebook</a>
                  <a href="https://www.instagram.com/muttonlane/" target="_blank" rel="noopener noreferrer" style={{ color: SMOKE, textDecoration: "none", fontSize: 13 }}>📸 Instagram</a>
                </div>
              </div>
            </div>
          </Fade>
      ...
      <footer style={{ background: "#060402", borderTop: "2px solid rgba(200,120,10,0.18)", padding: "2rem 20px", textAlign: "center" }}>
        <p style={{ color: "rgba(244,234,216,0.18)", fontSize: 12, letterSpacing: "0.15em", marginBottom: 16 }}>© 2025 MUTTON LANE INN · 3 MUTTON LANE · CORK · EST. 1787</p>
        <div style={{ display: "flex", justifyContent: "center", gap: 28, flexWrap: "wrap" }}>
          <a href="https://maps.google.com/?q=Mutton+Lane+Inn+3+Mutton+Lane+Cork" target="_blank" rel="noopener noreferrer" style={{ color: "#999", textDecoration: "none", fontSize: 14 }}>📍 Google Maps</a>
          <a href="https://www.facebook.com/mutton.lane/" target="_blank" rel="noopener noreferrer" style={{ color: "#999", textDecoration: "none", fontSize: 14 }}>📘 Facebook</a>
          <a href="https://www.instagram.com/muttonlane/" target="_blank" rel="noopener noreferrer" style={{ color: "#999", textDecoration: "none", fontSize: 14 }}>📸 Instagram</a>
        </div>
      </footer>
